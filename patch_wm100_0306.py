...
if __name__ == "__main__":
    ...
    print("")
    print("############################################################################")
    print("this patcher is supposed to be used on the 0306 module of Spark 1.00.0900 fw")
    ...
    #
    #   original Spark 0306 version is 03.02.43.20 = 142B0203

    # HardCoded version of the version in some FC function    
    p.patchByteAtAddress(0x521854,hex_parts[0])  #0x14 = 20
    p.patchByteAtAddress(0x521856,hex_parts[1])  #0x2B = 43
    p.patchByteAtAddress(0x521862,hex_parts[2])  #0x2 = 2
    p.patchByteAtAddress(0x521864,hex_parts[3])  #0x3 = 3

    newversioncode = sn_hex
    print("New version code : %s\n" % newversioncode)
    p.patchDataAtAddress(0x4F9344,newversioncode)
    p.patchDataAtAddress(0x50B220,newversioncode)
    p.patchDataAtAddress(0x521AA0,newversioncode)
    p.patchDataAtAddress(0x52EA40,newversioncode)

    # Ascii version of the version string    
    p.patchStringAtAddress(0x58C005,sn)
    ...
    #
    p.patchFloatAtAddress(0x4ADE18,-5000.0)  #   Min altitude relative to home point : default -200.0 m
    p.patchFloatAtAddress(0x4ADE1C,9000.0)   #   Max altitude relative to home point : default 1000.0 m
    p.patchFloatAtAddress(0x4ADE64,32000.0)  #   Max distance from one waypoint to home point : default 2000.0 m 
    p.patchFloatAtAddress(0x4AE2D8,128000.0) #   Max total length of mission : default 30000.0 m
    p.patchFloatAtAddress(0x4AE32C ,25.0)    #   Max speed (positive value) : default  15.0 m/s
    p.patchFloatAtAddress(0x4AE330,-25.0)    #   Max speed (negative value) : default -15.0 m/s
    p.patchFloatAtAddress(0x4AE334,-5000.0)  #   Min altitude relative to home point : default -200.0 m
    p.patchFloatAtAddress(0x4AE338,9000.0)   #   Max altitude relative to home point : default 1000.0 m
    p.patchFloatAtAddress(0x4AE9B0,25.0)     #   Max speed for in-flight change speed message : default 15.0 m/s
    ...
    #ROM:004E410A                 CMP             R3, #0x13
    #ROM:004E410C                 BNE             loc_4E4114
    #ROM:004E410E                 LDRB            R1, [R1,#3]
    #ROM:004E4110                 CMP             R1, #0x20
    #ROM:004E4112                 BEQ             loc_4E4148    
    #p.patchDataAtAddress(0x4E410A,nop)
    #p.patchDataAtAddress(0x4E410C,nop)
    #p.patchDataAtAddress(0x4E4110,"FF29")    #  CMP R1,#20 -> CMP R1,#0xFF 
    #p.patchDataAtAddress(0x4E4112,"19D1")    #  BEQ -> BNE

    #    Patch the test for 0xB5 0x62 0x13 0x20 for saving timeid
    
    #ROM:004E3F0A                 CMP             R0, #0x13 ; jumptable 004E3EEA case 2
    #ROM:004E3F0C                 BNE             loc_4E3F06
    #ROM:004E3F12                 CMP             R0, #0x20 ; jumptable 004E3EEA case 3    
    #ROM:004E3F14                 BNE             loc_4E3F06
    #p.patchDataAtAddress(0x4E3F0A,nop)
    #p.patchDataAtAddress(0x4E3F0C,nop)
    #p.patchDataAtAddress(0x4E3F12,nop)
    #p.patchDataAtAddress(0x4E3F14,nop)
    ...
    p.patchAddressAtAddress(0x51DF14,unmodded_firmware_end)

    #   Write new UBX frames at beginning of new page
    p.patchDataAtAddress(unmodded_firmware_end,newubxframes)

    #            ROM:0051DB32                 LDR             R0, [R0,#8]
    # TO PATCH : ROM:0051DB34                 MOVS            R2, #8
    #            ROM:0051DB36                 LDR             R1, =dword_569C70
    #            ROM:0051DB38                 B.W             sub_42A398
    #            ROM:0051DB3C ; ---------------------------------------------------------------------------
    #            ROM:0051DB3C                 LDR             R1, =dword_569C70
    # TO PATCH : ROM:0051DB3E                 MOVS            R2, #0x15
    #            ROM:0051DB40                 LDR             R0, [R0,#8]
    # TO PATCH : ROM:0051DB42                 ADDS            R1, #8
    #            ROM:0051DB44                 B.W             sub_42A398
    ...
    p.patchByteAtAddress(0x51DB34,length_part1)
    p.patchByteAtAddress(0x51DB3E,length_part2)
    p.patchByteAtAddress(0x51DB42,length_part1)

    #    Disable QCchecking instructions about LLH / ECEF mismatch
    
    #p.patchDataAtAddress(0x51E648,nop)
    #p.patchDataAtAddress(0x51EB00,nop)
    #p.patchDataAtAddress(0x51EBEE,nop)
