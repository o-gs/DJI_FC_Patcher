...
if __name__ == "__main__":
    ...
    print("")
    print("################################################################################")
    print("this patcher is supposed to be used on the 0306 module of P4P 1.05.0600 fw")
    ...
    #
    #   original P4P 0306 version is 03.02.44.07 = 072C0203

    # HardCoded version of the version in some FC function
    p.patchByteAtAddress(0x524C78,hex_parts[0])  #0x07 = 07
    p.patchByteAtAddress(0x524C7A,hex_parts[1])  #0x2C = 44
    p.patchByteAtAddress(0x524C86,hex_parts[2])  #0x2 = 2
    p.patchByteAtAddress(0x524C88,hex_parts[3])  #0x3 = 3
    
    newversioncode = sn_hex
    print("New version code : %s\n" % newversioncode)
    p.patchDataAtAddress(0x4FB1C0,newversioncode)
    p.patchDataAtAddress(0x50D4A8,newversioncode)
    p.patchDataAtAddress(0x524EC4,newversioncode)
    p.patchDataAtAddress(0x5311A8,newversioncode)

    # Ascii version of the version string
    p.patchStringAtAddress(0x58EB2E,sn)
    ...
    #
    p.patchFloatAtAddress(0x4AEE68,-5000.0)  #   Min altitude relative to home point : default -200.0 m OK P4P
    p.patchFloatAtAddress(0x4AEE6C,9000.0)   #   Max altitude relative to home point : default 1000.0 m OK P4P
    p.patchFloatAtAddress(0x4AEEB4,32000.0)  #   Max distance from one waypoint to home point : default 2000.0 m  OK P4P
    p.patchFloatAtAddress(0x4AF328,128000.0) #   Max total length of mission : default 30000.0 m   OK P4P    
    p.patchFloatAtAddress(0x4AF37C ,25.0)    #   Max speed (positive value) : default  15.0 m/s OK P4P
    p.patchFloatAtAddress(0x4AF380,-25.0)    #   Max speed (negative value) : default -15.0 m/s OK P4P
    p.patchFloatAtAddress(0x4AF384,-5000.0)  #   Min altitude relative to home point : default -200.0 m OK P4P
    p.patchFloatAtAddress(0x4AF388,9000.0)   #   Max altitude relative to home point : default 1000.0 m OK P4P
    p.patchFloatAtAddress(0x4AFA00,25.0)     #   Max speed for in-flight change speed message : default 15.0 m/s OK P4P
    ...
    p.patchAddressAtAddress(0x521324,unmodded_firmware_end)

    #   Write new UBX frames at beginning of new page
    p.patchDataAtAddress(unmodded_firmware_end,newubxframes)

    #            ROM:00520F42                 LDR             R0, [R0,#8]
    # TO PATCH : ROM:00520F44                 MOVS            R2, #8
    #            ROM:00520F46                 LDR             R1, =UBLOX_FRAMES
    #            ROM:00520F48                 B.W             sub_42A37C
    #            ROM:00520F4C ; ---------------------------------------------------------------------------
    #            ROM:00520F4C                 LDR             R1, =UBLOX_FRAMES
    # TO PATCH : ROM:00520F4E                 MOVS            R2, #0x15
    #            ROM:00520F50                 LDR             R0, [R0,#8]
    # TO PATCH : ROM:00520F52                 ADDS            R1, #8
    #            ROM:00520F54                 B.W             sub_42A37C
    ...
    p.patchByteAtAddress(0x520F44,length_part1)
    p.patchByteAtAddress(0x520F4E,length_part2)
    p.patchByteAtAddress(0x520F52,length_part1)

    #    Disable QCchecking instructions about LLH / ECEF mismatch
    
    #p.patchDataAtAddress(0x521A58,nop)
    #p.patchDataAtAddress(0x521F10,nop)
    #p.patchDataAtAddress(0x521FFE,nop)
