...
if __name__ == "__main__":
    ...
    print("")
    print("################################################################################")
    print("this patcher is supposed to be used on the 0306 module of P4 standard 2.00.0700 fw")
    ...
    p.patchByteAtAddress(0x524BD0,hex_parts[0])  #0x07 = 07
    p.patchByteAtAddress(0x524BD2,hex_parts[1])  #0x2C = 44
    p.patchByteAtAddress(0x524BDE,hex_parts[2])  #0x2 = 2
    p.patchByteAtAddress(0x524BE0,hex_parts[3])  #0x3 = 3

    newversioncode = sn_hex
    print("New version code : %s\n" % newversioncode)
    p.patchDataAtAddress(0x4FB148,newversioncode)
    p.patchDataAtAddress(0x50D400,newversioncode)
    p.patchDataAtAddress(0x524E1C,newversioncode)
    p.patchDataAtAddress(0x531100,newversioncode)

    # Ascii version of the version string
    p.patchStringAtAddress(0x58EA84,sn)
    ...
    #
    p.patchFloatAtAddress(0x4AEDE8,-5000.0)  #   Min altitude relative to home point : default -200.0 m OK P4
    p.patchFloatAtAddress(0x4AEDEC,9000.0)   #   Max altitude relative to home point : default 1000.0 m OK P4    
    p.patchFloatAtAddress(0x4AEE34,32000.0)  #   Max distance from one waypoint to home point : default 2000.0 m  OK P4
    p.patchFloatAtAddress(0x4AF2A8,128000.0) #   Max total length of mission : default 30000.0 m   OK P4
    p.patchFloatAtAddress(0x4AF2FC ,25.0)    #   Max speed (positive value) : default  15.0 m/s OK P4
    p.patchFloatAtAddress(0x4AF300,-25.0)    #   Max speed (negative value) : default -15.0 m/s OK P4    
    p.patchFloatAtAddress(0x4AF304,-5000.0)  #   Min altitude relative to home point : default -200.0 m OK P4
    p.patchFloatAtAddress(0x4AF308,9000.0)   #   Max altitude relative to home point : default 1000.0 m OK P4
    p.patchFloatAtAddress(0x4AF980,25.0)     #   Max speed for in-flight change speed message : default 15.0 m/s OK P4
    ...
    p.patchAddressAtAddress(0x52127C,unmodded_firmware_end)
    ...
    p.patchByteAtAddress(0x520E9C,length_part1)
    p.patchByteAtAddress(0x520EA6,length_part2)
    p.patchByteAtAddress(0x520EAA,length_part1)

    #    Disable QCchecking instructions about LLH / ECEF mismatch
    
    #p.patchDataAtAddress(0x5219B0,nop)
    #p.patchDataAtAddress(0x521E68,nop)
    #p.patchDataAtAddress(0x521F56,nop)    
